...
for problem in (3, 4):
    ...
    if problem == 3:
        ...
        specs = 'ccrrr'
    lines = [
        r'\begin{table}[H]\centering\caption{问题' + str(problem)
        + r'的15场离线固定输入结果}\label{tab:q' + str(problem) + '}',
        r'\small\setlength{\tabcolsep}{7pt}',
        r'\begin{tabular}{' + specs + r'}\toprule', header + r'\\\midrule'
    ]
    for case in rows:
...
for problem in (3, 4):
    for i, row in enumerate([r for r in data['practice'] if r['problem'] == problem], 1):
        lines.append(
