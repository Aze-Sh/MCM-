...
def source_files(root=ROOT):
    paths = [root / name for name in (
        'run_robot.py', 'solve_questions.py', 'start_robot.ps1', 'check.py',
        'prepare_submission.py')]
    for directory in ('src/jammer_solver', 'tools', 'tests', 'validation/baseline_v7', 'paper'):
        paths.extend(sorted((root / directory).glob('*.py')))
    return paths


def manifest(root=ROOT):
    return [dict(path=p.relative_to(root).as_posix(),
...
def generate(root=ROOT):
    ...
    for record in records:
        name = record['path']
        lines += [r'\subsection*{\texttt{\detokenize{' + name + '}}}',
                  f'文件行数：{record["lines"]}。']
        if record['lines']:
            lines.append(r'\VerbatimInput[fontsize=\fontsize{8.5}{10.5}\selectfont,'
