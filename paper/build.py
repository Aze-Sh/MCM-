...
def input_hashes(root):
    ...
    paths.update(
        ...
        for name in (
            "paper/results.json",
            "paper/figure-data.json",
            "submission_inputs/formal_results.csv",
            "submission_inputs/human_review.json",
        )
    )
    return {
...
def main():
    ...
    if engine is None:
        raise SystemExit(
            "Install TeX Live or MiKTeX with XeLaTeX, ctex, fvextra and Fandol fonts."
        )
    formal_tables(root)
    ...
    for source, destination in [
        ...
        for _ in range(2):
            with (build / f"{source}-compile.log").open(
                "w", encoding="utf-8"
            ) as output:
            ...
            if completed.returncode:
                raise SystemExit(
                    f"Compile failed; inspect paper/.build/{source}-compile.log"
                )
        log = (build / f"{source}.log").read_text(encoding="utf-8", errors="replace")
        defects = re.findall(
            r".*(?:Overfull|Missing character|undefined references|undefined on input).*",
            log,
        )
        if defects:
            raise SystemExit("\n".join(defects))
        pages = int(
            re.search(r"Output written on .*?\((\d+) pages?", log, re.S).group(1)
        )
        body_pages = None
        if source == "main":
            ...
            if body_pages > 30 or pages <= body_pages:
                raise SystemExit(
                    f"Main text is {body_pages} pages; total is {pages}. Check the page limit and appendix."
                )
        ...
        report["documents"].append(
            dict(
                file=destination,
                pages=pages,
                body_and_references_pages=body_pages,
        ...
        print(
            f"Built {target.name}: {pages} pages; main text and references: {body_pages}"
        )
    report["input_sha256"] = input_hashes(root)
    (paper / "build-report.json").write_text(
