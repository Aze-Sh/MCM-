...
if font_family is None:
    raise SystemExit(
        "Set MCM_PAPER_FONT to an installed Chinese TrueType font before regenerating figures."
    )
plt.rcParams.update(
    {
...
def save(fig, name):
    fig.savefig(PAPER / "figures" / f"{name}.pdf", bbox_inches="tight", pad_inches=0.04)
    plt.close(fig)
...
region = jhqy(obs)
poly = np.array(region["vertices"])
ax = axs[0]
ax.add_patch(Polygon(poly, facecolor="#DCECF5", edgecolor=BLUE, lw=2))
a, b = np.array(region["farthest_pair"])
ax.plot([a[0], b[0]], [a[1], b[1]], "o--", color=RED, ms=4)
ax.set_xlim(47.5, 52.5)
ax.set_ylim(47.5, 52.5)
equal(ax)
ax.set_title("(a) 两次观测的交会区域")
ax.text(50, 47.8, "D = 3.5096 m", ha="center", fontsize=9)
ax = axs[1]
tri = np.array([[-0.5, 0], [0.5, 0], [0, np.sqrt(3) / 2]])
ax.add_patch(Polygon(tri, fill=False, edgecolor="black", lw=2))
ax.add_patch(Circle((0, 0), 0.5, fill=False, ls="--", color=RED, lw=2))
ax.add_patch(
    Circle((0, np.sqrt(3) / 6), 1 / np.sqrt(3), fill=False, ls="-.", color=BLUE, lw=2)
)
ax.scatter(*tri.T, color="black", s=20)
ax.set_xlim(-0.8, 0.8)
ax.set_ylim(-0.65, 1)
equal(ax, "x / d", "y / d")
ax.set_title("(b) 同直径圆的反例")
ax.plot([], [], "--", color=RED, label="半径 d/2")
ax.plot([], [], "-.", color=BLUE, label=r"半径 $d/\sqrt{3}$")
...
for ax, problem in zip(axs, [3, 4]):
    ...
    if problem == 3:
        for p in q:
            ax.add_patch(Circle(p, 1000, fill=False, color=BLUE, lw=0.65, alpha=0.45))
    else:
        ax.plot(*q[1:9].T, "o", color=BLUE, ms=4)
        ax.plot(*q[9:].T, "s", color=RED, ms=4)
    ax.plot(q[:, 0], q[:, 1], "o", color=BLUE, ms=3.5)
    ...
    ax.set_title(f"问题{problem}：{len(q)}个搜索站")
save(fig, "coverage_layouts")

fig, axs = plt.subplots(1, 2, figsize=(6.3, 2.95), layout="constrained")
ax = axs[0]
source = np.array([0.0, 0.0])
witness = np.array([[-450, -300], [500, -200], [150, 550]])
ax.add_patch(Polygon(witness, facecolor="#DCECF5", edgecolor=BLUE, lw=2))
ax.scatter(*witness.T, color=BLUE, s=35)
ax.plot(0, 0, "*", ms=12, color=RED)
ax.fill_between([-650, 700], -600, 600, color=GREEN, alpha=0.05)
...
ax.add_patch(
    Polygon(
        [a, (1250, -1000 / 3), (1250, 1000 / 3), b],
        facecolor="#F5DFD4",
        edgecolor=RED,
        lw=2,
    )
)
ax.plot([0, 1300], [0, 0], ":", color=GRAY)
ax.plot(0, 0, "*", ms=11, color=GREEN)
ax.plot([750, 750], [-200, 200], "s", color=RED)
ax.text(60, 60, "实际阳性锚点", fontsize=9)
ax.text(1020, -80, "可排除部分", ha="center", fontsize=9)
ax.text(780, 360, "双阴性测点", fontsize=9)
...
def local(point):
    delta = np.array((tuple((float(x) for x in point)))) - origin
    return np.array([delta @ axis, delta @ normal])


fig, ax = plt.subplots(figsize=(6.3, 2.35), layout="constrained")
...
for i, q in enumerate(points):
    ...
    if part:
        ax.add_patch(
            Polygon(
                [local(v) for v in part], facecolor=color, edgecolor=color, alpha=0.23
            )
        )
    ax.plot(*qf, "o", color=color, ms=4)
    ax.annotate(str(i + 1), qf, xytext=(5, 4), textcoords="offset points", fontsize=9)
qs = np.array([local(q) for q in points])
ax.plot(qs[:, 0], qs[:, 1], "-", color=GRAY, lw=1.2)
ax.add_patch(Polygon([local(v) for v in poly], fill=False, color="black", lw=2))
ax.autoscale_view()
...
for j, p in enumerate([3, 4]):
    ...
    costs = [
        ...
        np.mean([r["counts"]["measure"] * 5 for r in rows]),
        np.mean([r["counts"]["switch"] for r in rows]),
        np.mean(
            [
                r["counts"]["successful_clear"] * 5
                + (r["counts"]["clear"] - r["counts"]["successful_clear"]) * 3
                for r in rows
            ]
        ),
    ]
    ...
    for i, (value, color, label) in enumerate(
        zip(costs, [BLUE, RED, GREEN, PURPLE], ["移动", "测向", "切频", "光学与清除"])
    ):
        axs[1].bar(
            j,
            value,
        ...
        base += value
axs[1].set_xticks([0, 1], ["问题3", "问题4"])
axs[1].set_ylabel("平均虚拟时间 / s")
axs[1].legend(fontsize=8, ncol=2, loc="upper left")
axs[1].set_ylim(0, 8200)
...
for ax, p in zip(axs, [3, 4]):
    ...
    x = [
        r["reported_source_count"] if p == 3 else r["reported_directional"]
        for r in rows
    ]
    ...
    for i, (xx, yy) in enumerate(zip(x, y)):
        ax.annotate(
            str(i + 1), (xx, yy), xytext=(5, 4), textcoords="offset points", fontsize=9
        )
    ax.set_xlabel("截图报告源个数" if p == 3 else "截图报告定向源个数")
    ax.set_ylabel("截图报告总时间 / s")
