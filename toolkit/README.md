...
本目录是可独立使用的通用建模工具箱。两套 B 题方案与题目资料位于上一级仓库，见 [仓库首页](../README.md)。下文相对路径均以 `toolkit/` 为基准。

从仓库根目录先执行 `cd toolkit`，再运行下面的 PowerShell 命令：
...
$project = Join-Path (Get-Location) "tmp/new-project"
python tools/init_project.py $project --languages python matlab
...
- [第一套方案](../solutions/baseline/README.md)：覆盖扫描基线、模拟器、论文、演练记录和 v3 冻结代码。
- [第二套方案](../solutions/adaptive/README.md)：自适应几何与任务规划，默认 v7。
- [共用题目资料](../problem/README.md)。
