    ...
    for event in events:
        ...
        if kind == "v8_started":
            ...
            for q in points:
                ...
                if cell and not g.ypbh(cell, q, F("19.8")):
                    raise ValueError(
                        "Optical chain leaves feasible source positions uncovered"
                    )
            previous = g.point(position)
            distance = F(0)
            for q in points:
                distance += g.jlsx(previous, q)
                previous = q
            found = min(16, len(v8.pindao(ledger, ("found",))) + len(points))
            required = (
                virtual
                + v8.sysj(ledger, position)
                + float((2 * found + 2) * distance / 5)
                + 6 * len(points)
            )
            if ledger["extra"] < len(points) or required >= limit - 2:
                raise ValueError(
                    "The complete optical event does not fit its reserved budget"
                )
            if v8.lxsj(points, position) > event["upper_s"] + 1e-6:
            ...
            active = dict(
                ...
                points=points,
                count=0,
                upper_s=event["upper_s"],
                budget_required_s=required,
            )
            ...
            if active is not None:
                ...
                if (
                    event["path"] != "/clear"
                    or event["channel"] != active["channel"]
                    or index >= len(active["points"])
                    or g.point(q) != active["points"][index]
                ):
                    raise ValueError(
                        "Execution departed from the certified optical chain"
                    )
                active["count"] += 1
            v8.gxjl(
                ledger,
            ...
            if active is not None and event["reply"]["clear_result"] == "success":
                ...
                if elapsed > active["upper_s"] + 1e-5:
                    raise ValueError(
                        "The actual optical prefix exceeded its upper bound"
                    )
                chains.append(
                    dict(
                        channel=active["channel"],
                        points=len(active["points"]),
                        actions=active["count"],
                        actual_s=elapsed,
                        upper_s=active["upper_s"],
            ...
            v8.syx(
                ledger["channels"][event["channel"]]["source"],
                g.point(event["positive"]),
                g.point(event["a"]),
                g.point(event["b"]),
            )
    if active is not None or ledger is None or not v8.qbqc(ledger):
