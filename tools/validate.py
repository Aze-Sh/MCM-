...
def verify(record, result, events, log, policy_hashes):
    ...
    if (
        result["policy_source_sha256"] != policy_hashes
        or digest(log) != result["log_sha256"]
    ):
        raise ValueError(f"Policy or log hash differs: {record['name']}")
    requests = [
        (e["path"], e["request"])
        for e in events
        if e.get("event") == "offline_response"
    ]
    ...
    if request_hash != record["request_sha256"]:
        raise ValueError(
            f"Requests changed from the selected v8 fast version: {record['name']}"
        )
    if (
        abs(result["virtual_time_s"] - record["virtual_time_s"]) > 1e-6
        or result["counts"] != record["counts"]
    ):
        raise ValueError(
            f"Costs changed from the selected v8 fast version: {record['name']}"
        )
...
def main():
    ...
    parser.add_argument(
        "--suite",
        default="all",
        choices=("all", "matched", "validation", "fresh", "stress", "fallback"),
    )
    ...
    if args.output is None:
        args.output = (
            ROOT / "validation" / "current"
            if args.verify_only
            else ROOT / "runs" / f"validation-{datetime.now():%Y%m%d-%H%M%S-%f}"
        )
    args.output.mkdir(parents=True, exist_ok=True)
    frozen = json.loads(
        (ROOT / "validation/expected-fast.json").read_text(encoding="utf-8")
    )
    records = [
        r for r in frozen["cases"] if args.suite == "all" or r["suite"] == args.suite
    ]
    if not records:
        raise ValueError("No frozen cases selected")
    policy_hashes = {
        p.name: digest(p) for p in sorted(Path(solver.__file__).parent.glob("*.py"))
    }
    checked = []
    for record in records:
        ...
        if args.verify_only:
            ...
            events = [
                json.loads(line)
                for line in path.with_suffix(".jsonl")
            ...
            result = json.loads(path.read_text(encoding="utf-8"))
        checked.append(
            verify(record, result, events, path.with_suffix(".jsonl"), policy_hashes)
        )
        print(
            json.dumps(
                {
    ...
    report = dict(
        scope="Offline synthetic only; no official simulator connected",
        ...
        cases=checked,
        verified_cases=len(checked),
        optical_chains=sum(len(r["optical_audit"]["chains"]) for r in checked),
        identical_requests_to_selected_fast=all(
            r["identical_requests"] for r in checked
        ),
    )
