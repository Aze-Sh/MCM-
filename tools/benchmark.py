...
from jammer_solver.verification import replay, calibrate
...
def case_from_record(record):
    return dict(name=record['name'], problem=record['problem'], seed=record['seed'],
                extreme=record['extreme'],
                sources=[Source(s['channel'], tuple(s['position']), s['radius'], s['direction'])
