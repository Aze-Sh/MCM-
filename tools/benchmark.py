...
from simulator import SyntheticSimulator, Source
from jammer_solver import solver as v8
from verification import replay, calibrate
...
def offline_io(send):
    io = dict(
        ...
        events=[],
        counts=Counter(),
        wall_deadline=math.inf,
    )

    def call(path, point=None, channel=None):
        if time.monotonic() >= io["wall_deadline"]:
            raise TimeoutError("Offline wall time limit exceeded")
        io["serial"] += 1
        request = dict(
            arena_id="default",
            robot_id="offline-v8",
            request_id=f"offline-{io['serial']}",
        )
        if point is not None:
            request.update(
                position=dict(x=float(point[0]), y=float(point[1])), channel=channel
            )
        status, reply = send(path, json.dumps(request).encode())
        io["events"].append(
            dict(
                event="offline_response",
                path=path,
                request=request,
                reply=reply,
                status=status,
    ...
    def record(event):
        if time.monotonic() >= io["wall_deadline"]:
            raise TimeoutError("Offline wall time limit exceeded")
        io["counts"][event["event"]] += 1
        io["events"].append(event)

    io.update(call=call, record=record)
    return io
...
class ExtremeSimulator(SyntheticSimulator):
    def _detect(self, source, pos):
        ...
        if reply["measure_result"] == "direction":
            angle = math.degrees(
                math.atan2(source.position[1] - pos[1], source.position[0] - pos[0])
            )
...
def case_from_record(record):
    return dict(
        ...
        seed=record["seed"],
        extreme=record["extreme"],
        sources=[
            Source(s["channel"], tuple(s["position"]), s["radius"], s["direction"])
            for s in record["source_truth"]
        ],
    )
...
def cases(suite):
    if suite == "matched":
        saved = json.loads((ROOT / "validation" / "expected-fast.json").read_text())
        return [case_from_record(c) for c in saved["cases"] if c["suite"] == "matched"]
    if suite == "stress":
        result = []
        for problem in (3, 4):
            for n in (10, 16):
                for geometry in ("boundary", "cluster"):
                    ...
                    for i in range(n):
                        ...
                        sources.append(
                            Source(i + 1, p, 1000.0, angle if problem == 4 else None)
                        )
                    result.append(
                        dict(
                            name=f"q{problem}-{geometry}-n{n}",
                            problem=problem,
                            seed=n,
                            sources=sources,
                            extreme=True,
    ...
    if suite in ("validation", "fresh"):
        ...
        for problem in (3, 4):
            for n in (10, 12, 14, 16):
                seed = (
                    (20261000 if suite == "validation" else 20261100) + problem * 20 + n
                )
            ...
            dict(
                ...
                sources=[
                    Source(
                        ...
                        ((i - 5) * 35.0, (i % 3 - 1) * 25.0),
                        1000.0,
                        (i * 37) % 360 if p == 4 else None,
                    )
                    for i in range(n)
                ],
                extreme=True,
            )
            for p, n in ((3, 10), (4, 16))
        ]
...
def run_case(
    ...
    result = dict(
        ...
        source_truth=[asdict(s) for s in case["sources"]],
        noise="fixed extreme +/-1 degree"
        if case.get("extreme")
        else "repository fixed coordinate hash",
    ...
    try:
        result["summary"] = v8.yunxing(solver)
        if algorithm == "v8":
            audit = replay(io["events"])
            result["replay"] = {
                k: audit[k] for k in ("verified", "actions", "virtual_time_s")
            }
        ...
        exited = solver["exit_confirmed"]
        virtual = solver["virtual"]
        if len(world.cleared) != len(case["sources"]) or not complete or not exited:
            raise AssertionError(
                "Solver failed actual full clearance or declared completion"
            )
        ...
        result["summary"] = v8.huizong(solver)
    finally:
        if has_alarm:
            signal.alarm(0)
    ...
    result.update(
        ...
        average_clear_time_s=world.virtual_time / len(case["sources"]),
        counts=world.counts,
        event_counts=dict(io["counts"]),
    )
...
def main():
    ...
    parser.add_argument(
        "--suite",
        choices=("matched", "stress", "fallback", "calibration", "validation", "fresh"),
        default="matched",
    )
    parser.add_argument("--algorithm", choices=("v8",), default="v8")
    parser.add_argument("--planning-seconds", type=float, default=0.20)
    parser.add_argument("--extra-actions", type=int, default=640)
    parser.add_argument("--wall-limit", type=int, default=90)
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "offline")
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=True)
    if args.suite == "calibration":
        result = [
            calibrate([(0,), (60,), (120,)], (0, 60, 120)),
    ...
    for case in cases(args.suite):
        result, _ = run_case(
            case,
            args.algorithm,
            args.planning_seconds,
            args.extra_actions,
            fallback_only=args.suite == "fallback",
    ...
    (args.output / f"{args.algorithm}-{args.suite}.json").write_text(
