...
def main():
    ...
    reference = json.loads(
        (ROOT / "validation/performance-comparison.json").read_text(encoding="utf-8")
    )
    inputs = {
        r["name"]: r
        for r in json.loads(
    ...
    for row in rows:
        ...
        adapter = SimpleNamespace(
            call=io["call"], record=io["record"], pending=None, deadline=math.inf
        )
        start = time.monotonic()
        solver = SolverV7(adapter, case["problem"])
        solver.run()
        total = world.virtual_time
        matches = abs(total - row["v7_s"]) < 1e-6 and len(world.cleared) == len(
            case["sources"]
        )
        result = dict(
            name=row["name"],
            ...
            expected_time_s=row["v7_s"],
            matches=matches,
            wall_s=time.monotonic() - start,
            scope="offline synthetic comparison only",
        )
        ...
        logfile.with_suffix(".json").write_text(
    ...
    (args.output / "verification.json").write_text(
        json.dumps(dict(cases=checked, verified=len(checked)), indent=2) + "\n",
        encoding="utf-8",
    )
