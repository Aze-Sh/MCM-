...
class SyntheticSimulator:
    ...
    def _detect(self, source, pos):
        ...
        if source.direction is not None:
            ...
            outward = (pos[0] - source.position[0]) * math.cos(angle) + (
                pos[1] - source.position[1]
            ) * math.sin(angle)
        ...
        error = (
            int.from_bytes(hashlib.sha256(token).digest()[:8], "big") / (2**64 - 1) * 2
            - 1
        )
        return dict(
            measure_result="direction",
            svd_deg=round((true_angle + error) % 360, 2) % 360,
        )
...
def random_case(seed, mixed=False):
    ...
    for channel in rng.sample(range(1, 21), count):
        ...
        direction = rng.random() * 360 if mixed and rng.random() < 0.6 else None
        sources.append(
            Source(
                channel,
