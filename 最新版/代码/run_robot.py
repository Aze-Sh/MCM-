...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path(__file__).resolve().parents[1] / "运行结果")
    ...
    metadata = dict(problem=args.problem, case_code=args.case_code, run_kind=args.run_kind,
                    strategy=("midpoint-v1" if args.baseline else "annular-history-route-v3" if args.v3
                              else V4_STRATEGY if args.v4 else V5_STRATEGY if args.v5 else V6_STRATEGY if args.v6 else STRATEGY),
