"""Exact rational predicates, with certified trigonometric enclosures.

Floating point is used to propose points or to reject expensive proof attempts.
Every accepted geometric exclusion is checked with rational arithmetic.
"""
...
EPS = F(201, 200)  # Includes half of the 0.01 degree output quantization.
DOMAIN = F(1800)
...
def clip(poly, normal, bound):
    """Closed half-plane; exact intersections retain boundary possibilities."""
    if not poly:
        return []
    out = []
    a = poly[-1]
    ...
    for b in poly:
        ...
        if fb <= 0:
            out.append(b)
        a, fa = b, fb
    # Consecutive duplicates are harmless but expensive downstream.
    result = []
    for p in out:
...
def contains(poly, p):
    ...
    if len(poly) == 1:
        return p == poly[0]
    if len(poly) == 2:
        return cross(sub(poly[1],poly[0]),sub(p,poly[0])) == 0 and dot(sub(p,poly[0]),sub(p,poly[1])) <= 0
    return all(cross(sub(b,a),sub(p,a)) >= 0 for a,b in zip(poly,poly[1:]+poly[:1]))


@lru_cache(maxsize=4096)
def trig(degrees):
    """Return certified (cos_lo, cos_hi, sin_lo, sin_hi).

    Reduce to [-pi/4,pi/4]. Exact Taylor sums with a Lagrange remainder
    enclose sin/cos at the rational midpoint; both functions are 1-Lipschitz.
    Round OUTWARDS onto a 1e-12 rational lattice to keep predicates cheap.
    """
    degrees = F(degrees)
    ...
    # |x| < 1. Taylor degrees 19 and 18, respectively.
...
def uncertain_plane(poly, origin, normal, error, rhs=F(0)):
    """True source lies in [-1800,1800]^2: bound normal error there."""
...
def bearing_constraints(origin, lo, hi=None):
    """Constraints for a whole, UNWRAPPED output interval, not its midpoint."""
    hi = lo if hi is None else hi
    if F(hi)-F(lo)+2*EPS >= 180:
        return []
    ulo,elo = direction(F(lo)-EPS)
    uhi,ehi = direction(F(hi)+EPS)
    return [((ulo[1],-ulo[0]),(elo[1],elo[0]),F(0)),
...
def apply_bearing(poly, origin, bearing, radius=F(1500), hi=None):
    ...
    # Reception also implies this (loose) square.
    for n,b in (((1,0),origin[0]+1500),((-1,0),1500-origin[0]),
                ((0,1),origin[1]+1500),((0,-1),1500-origin[1])):
        poly = clip(poly,n,b)
    return poly


def transformed_box(origin, bearing, bounds):
    """Outer polygon for the IDEAL rotation of a local rectangle."""
    u,e = direction(F(bearing))
...
def verified_pair_cut(poly, positive, a, b):
    """Two negative in-range points, a previous positive, and a cone proof.

    Beyond line ab inside cone positive->(a,b), the positive ray is a
    nonnegative combination of both negative rays. No emitting half-plane
    can make that positive and both others strictly negative.
    """
    ...
    if dot(normal,positive)>bound:
        normal = (-normal[0],-normal[1]);bound = -bound
...
def coverage_certificate(radio, optical, problem, max_nodes=2048, max_depth=12):
    ...
    while stack and nodes < max_nodes:
        ...
        if optical_hit is not None:
            leaves.append(("optical",box,[optical_hit]));continue
        # Float filter only rejects possible certificates. Accepted tests exact.
        bf = [floating(v) for v in box]
        nearby = [q for q,qf in r_float if all(math.dist(qf,v)<1000.000001 for v in bf)
                  and disk_contains(box,q,F(1000))]
        if problem == 3 and nearby:
            leaves.append(("omni",box,nearby[:1]));continue
        if problem == 4 and len(nearby)>=3:
            h = hull(nearby)
            if all(contains(h,v) for v in box):
                leaves.append(("directional_hull",box,h));continue
        if depth >= max_depth:
            return False,nodes,tuple(map(float,(x0,y0,x1,y1))),()
        mx,my = (x0+x1)/2,(y0+y1)/2
        stack.extend((rectangle(*v),depth+1) for v in
